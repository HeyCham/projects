...
// Update user paddle position based on mouse movement or touch on mobile
canvas.addEventListener('mousemove', movePaddle);
...
function handleTouchMove(event) {
    ...
    const rect = canvas.getBoundingClientRect();
    const touchY = event.touches[0].clientY - rect.top - user.height / 2;
    const deltaY = touchY - user.y;

    // Adjust paddle position based on touch movement
    if (Math.abs(deltaY) > paddleSpeed) {
        user.y += deltaY > 0 ? paddleSpeed : -paddleSpeed;
    } else {
        user.y = touchY;
    }
...
// Render game on canvas
function render() {
    // Clear canvas with a black screen
    drawRect(0, 0, canvas.width, canvas.height, "BLACK");
    drawNet();
